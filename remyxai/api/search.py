...
class Asset:
    ...
    environment_vars: List[str] = field(
        default_factory=lambda: ["HF_TOKEN", "OPENAI_API_KEY"]
    )
...
def get_asset(arxiv_id: str, api_key: Optional[str] = None) -> Optional[Asset]:
...
def get_stats(api_key: Optional[str] = None) -> Dict[str, any]:
